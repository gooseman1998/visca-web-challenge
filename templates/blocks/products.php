<div class="products">
    <img src="./assets/images/product-background.svg" class="products__background-img" alt="product background figure">
    
    <div class="products__header">
        <div class="products__alphabetically">
            <span id="sort_alphabetically">Sort Alphabetically</span>
            <span id="sort_normal">Don't sort Alphabetically</span>
        </div>
        <div class="products__title-wrapper">
            <img src="./assets/images/Flag_of_Canada.svg" class="products__c-flag" alt="canada flag">
            <h1 class="products__title">BEST SPORTS BETTING SITES</h1>
        </div>
        <div class="products__change-view">
            <div class="custom-switch products__switch">
                <input type="checkbox" class="custom-control-input" id="viewSwitch">
                <label class="custom-control-label" for="viewSwitch">Change view</label>
            </div>
        </div>
    </div>
    <div class="row product-cards product-cards--list">
        
        <div class="col product-card product-card--list" id="sportsInteractionCard">
            <a href="#partner" class="product-card__link"></a>
            <span class="product-card__number">1</span>
            <div class="product-card__logo-wrapper">
                <a href="#partnerReviewImg" class="product-card__logo-link">
                    <img src="./assets/images/logos/sport-interaction-logo.svg" alt="product logo" class="product-card__logo">
                </a>
            </div>
            <div class="product-card__review-wrapper">
                <div class="product-card__stars">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                </div>
                <a href="#partnerReview" class="product-card__show-review product-card__show-review--desktop">Read review</a>
                <a href="#partnerReview" class="product-card__show-review product-card__show-review--mobile">Review</a>
            </div>
            <div class="product-card__sale-offer product-card__sale-offer--desktop"><span class="product-card__line-break"> <span class="product-card__sale-offer--bold"> - Exclusive - </span></span> Bonus up to <strong>$200</strong></div>
            <div class="product-card__sale-offer product-card__sale-offer--mobile"> <span class="product-card__line-break"> BONUS </span> <strong>$200</strong></div>
            <div class="btn btn-primary product-card__button product-card__button--desktop">Play Now <img src="./assets/images/misc/chevron.svg" class="product-card__button__chevron"></div>
            <div class="btn btn-primary product-card__button product-card__button--mobile"><img src="./assets/images/misc/chevron.svg" class="product-card__button__chevron product-card__button__chevron--mobile"></div>
        </div>
        <div class="col product-card product-card--list" id="powerPlaySportsCard">
            <a href="#partner" class="product-card__link"></a>
            <span class="product-card__number">2</span>
            <div class="product-card__logo-wrapper">
                <a href="#partnerReviewImg" class="product-card__logo-link">
                    <img src="./assets/images/logos/powerplay-logo.png" alt="product logo" class="product-card__logo">
                </a>
            </div>
            <div class="product-card__review-wrapper">
                <div class="product-card__stars">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                </div>
                <a href="#partnerReview" class="product-card__show-review product-card__show-review--desktop">Read review</a>
                <a href="#partnerReview" class="product-card__show-review product-card__show-review--mobile">Review</a>
            </div>
            <div class="product-card__sale-offer product-card__sale-offer--desktop"><span class="product-card__line-break"><span class="product-card__sale-offer--bold">- Exclusive - </span></span> Bonus up to <strong>$200</strong></div>
            <div class="product-card__sale-offer product-card__sale-offer--mobile"> <span class="product-card__line-break"> FREE BET </span> <strong>$250</strong></div>

            <div class="btn btn-primary product-card__button product-card__button--desktop">Play Now <img src="./assets/images/misc/chevron.svg" class="product-card__button__chevron"></div>
            <div class="btn btn-primary product-card__button product-card__button--mobile"><img src="./assets/images/misc/chevron.svg" class="product-card__button__chevron product-card__button__chevron--mobile"></div>
        </div>
        <div class="col product-card product-card--list" id="betwayCard">
            <a href="#partner" class="product-card__link"></a>
            <span class="product-card__number">3</span>
            <div class="product-card__logo-wrapper">
                <a href="#partnerReviewImg" class="product-card__logo-link">
                    <img src="./assets/images/logos/betway-logo.png" alt="product logo" class="product-card__logo">
                </a>
            </div>
            <div class="product-card__review-wrapper">
                <div class="product-card__stars">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                </div>
                <a href="#partnerReview" class="product-card__show-review product-card__show-review--desktop">Read review</a>
                <a href="#partnerReview" class="product-card__show-review product-card__show-review--mobile">Review</a>
            </div>
            <div class="product-card__sale-offer product-card__sale-offer--desktop"><span class="product-card__line-break">100% Sign Up Bonus</span> Up to <strong>$200</strong></div>
            <div class="product-card__sale-offer product-card__sale-offer--mobile"> <span class="product-card__line-break"> BONUS </span> <strong>$250</strong></div>

            <div class="btn btn-primary product-card__button product-card__button--desktop">Play Now <img src="./assets/images/misc/chevron.svg" class="product-card__button__chevron"></div>
            <div class="btn btn-primary product-card__button product-card__button--mobile"><img src="./assets/images/misc/chevron.svg" class="product-card__button__chevron product-card__button__chevron--mobile"></div>
        </div>
        <div class="col product-card product-card--list" id="spinSportsCard">
            <a href="#partner" class="product-card__link"></a>
            <span class="product-card__number">4</span>
            <div class="product-card__logo-wrapper">
                <a href="#partnerReviewImg" class="product-card__logo-link">
                    <img src="./assets/images/logos/spinsports-logo.svg" alt="product logo" class="product-card__logo">
                </a>
            </div>
            <div class="product-card__review-wrapper">
                <div class="product-card__stars">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                </div>
                <a href="#partnerReview" class="product-card__show-review product-card__show-review--desktop">Read review</a>
                <a href="#partnerReview" class="product-card__show-review product-card__show-review--mobile">Review</a>
            </div>
            <div class="product-card__sale-offer product-card__sale-offer--desktop"><span class="product-card__line-break"><span class="product-card__sale-offer--bold"> - Exclusive - </span></span>Up to <strong>$500</strong> Free Bet</div>
            <div class="product-card__sale-offer product-card__sale-offer--mobile"> <span class="product-card__line-break"> FREE BET </span> <strong>$200</strong></div>

            <div class="btn btn-primary product-card__button product-card__button--desktop">Play Now <img src="./assets/images/misc/chevron.svg" class="product-card__button__chevron"></div>
            <div class="btn btn-primary product-card__button product-card__button--mobile"><img src="./assets/images/misc/chevron.svg" class="product-card__button__chevron product-card__button__chevron--mobile"></div>
        </div>
        <div class="col product-card product-card--list"  id="bodogCard">
            <a href="#partner" class="product-card__link"></a>
            <span class="product-card__number">5</span>
            <div class="product-card__logo-wrapper">
                <a href="#partnerReviewImg" class="product-card__logo-link">
                    <img src="./assets/images/logos/bodog-logo.svg" alt="product logo" class="product-card__logo">
                </a>
            </div>
            <div class="product-card__review-wrapper">
                <div class="product-card__stars">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                    <img src="./assets/images/misc/star.svg" class="product-card__star">
                </div>
                <a href="#partnerReview" class="product-card__show-review product-card__show-review--desktop">Read review</a>
                <a href="#partnerReview" class="product-card__show-review product-card__show-review--mobile">Review</a>
            </div>
            <div class="product-card__sale-offer product-card__sale-offer--desktop"><span class="product-card__line-break">100% Sign Up Bonus</span> Up to <strong>$200</strong></div>
            <div class="product-card__sale-offer product-card__sale-offer--mobile"> <span class="product-card__line-break"> BONUS </span> <strong>$200</strong></div>

            <div class="btn btn-primary product-card__button product-card__button--desktop">Play Now <img src="./assets/images/misc/chevron.svg" class="product-card__button__chevron"></div>
            <div class="btn btn-primary product-card__button product-card__button--mobile"><img src="./assets/images/misc/chevron.svg" class="product-card__button__chevron product-card__button__chevron--mobile"></div>
        </div>
    </div>
    <div class="product-cards__view-more">
        <a href="#partnerReview" class="product-cards__view-more-link">
            View All Sports Betting Sites
        </a>
    </div>
</div>

